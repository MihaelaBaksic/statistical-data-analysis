...
Ljudske osobine i njihovo ispoljavanje često su istraživan fenomen. Trenutno dominantni model za klasifikaciju osobina ličnosti je Big Five model. Inicijalno je model bio razvijen s ciljem povezivanja osobina ličnosti i akademskih uspjeha i ponašanja. Barem četiri nezavisna skupa istraživača bavili su se problemom klasifikacija osobina ličnosti, te su svi diferencirali pet glavnih osobina ličnosti: ekstraverzija, ugodnost, savjesnost, neuroticizam i otvorenost. Bitno je naglasiti kako se ličnosti ne nalaze u ekstremima već na kontinuumu između ovih pet faktora.
...
```
Već iz priloženih grafova mogu naslutiti da su obje distribucije normalne te suprotno početnom pitanju izgleda da je savjesnost ispitanika istočne Azije manja nego u ostatku svijeta.
...
```
Na osnovu p vrijednosti od 5.76e-08 odbacujem nultu hipotezu u korist alternative da se savjesnost ispitanaika istočne Azije ne ravna po normalnoj distribuciji.
...
Pošto se nisam uvjerio u ispravnost mita da su stanovnici istočne Azije savjesniji od ostatka svijeta pitam postoji li uopće razlike u savjesnosti između stavnika različitih regija. Za primjer ću uzeti tri regije za koje imam sličan broj podataka. Moje pitanje glasi: "Postoji li razlika u savjesnosti između latinske Amerike,  sjeverne Afrike i zapadne Azije(arapskog svijeta) i centralne Azije"
...
```
Na osnovu p vrijednosti od 0.01102 odbacujem nultu hipotezu i zaključujem da varijance ove tri populacije nisu homogene. Ali za potrebe testiranja u nastavku pretpostavljam da jesu.

###Testiranje jednakosti savjesnosti

Provjeravamo postoje li razlike u savjesnosti između regija. Kao nultu hipotezu pretpostavljam da sve odabrane regije imaju jednako očekivanje razinu savjesnosti naspram alternativne hipoteze da nemaju to jest da barem jedan par regija nema jednako očekivanje za razinu savjesnosti.
...
Na osnovu p vrijednosti od 0.00167 odbacujem nultu hipotezu u korist alternative da očekivanje razine saavjesnosti nije isto za svaku odabranu regiju.
...
Postoje li razlike u karakteristikama prema spolu ispitanika? Možemo li temeljem nekih drugih varijabli odrediti spol ispitanika? Želimo ispitati da li postoje dovoljne razlike između razultata testiranja između pripadnika ratličitih spolova da bi mogli na temelju rezultata ispitivanja odrediti da li je test riješavala žena ili muškarac.

Kako bi znali predvidjeti spol ispitanika, možemo ispitati različite varijable koje bi mogle utjecati na spol:
...
Prvi korak je razvrstati podatke u dvije grupe muške i ženske konkretno u ovome skupu 1 predstavlja muški spol, dok 2 predstvavlje ženski spol.

```{r}
male_subjects = bigfive[bigfive$sex == 1 ,]
female_subjects = bigfive[bigfive$sex == 2 ,]
```
Zatim možemo usporediti srednje vrijednosti za 5 kategorija kako bi uočili po kojim karateristikama se naviše razlikuju spolovi:
```{r}
cat('Srednja vrijednost neurocizma  za pripadnice muškog/ženskog spola je ', mean(female_subjects$neuroticism_score), mean(male_subjects$neuroticism_score),'\n')

cat('Varijanca vrijednost neurocizma  za pripadnice muškog/ženskog spola je ', var(female_subjects$neuroticism_score),var(male_subjects$neuroticism_score), '\n')

cat('Srednja vrijednost ekstraverzije  za pripadnice muškog/ženskog spola je ', mean(female_subjects$extraversion_score), mean(male_subjects$extraversion_score),'\n')

cat('Varijanca vrijednost ekstraverzije  za pripadnice muškog/ženskog spola je ', var(female_subjects$extraversion_score),var(male_subjects$extraversion_score), '\n')

cat('Srednja vrijednost Ugodnosti za pripadnice muškog/ženskog spola je ', mean(female_subjects$agreeable_score), mean(male_subjects$agreeable_score),'\n')

cat('Varijanca vrijednost ugodnosti  za pripadnice muškog/ženskog spola je ', var(female_subjects$agreeable_score),var(male_subjects$agreeable_score), '\n')

cat('Srednja vrijednost Savjesnosti za pripadnice muškog/ženskog spola je ', mean(female_subjects$conscientiousness_score), mean(male_subjects$conscientiousness_score),'\n')

cat('Varijanca vrijednost Savjesnosti  za pripadnice muškog/ženskog spola je ', var(female_subjects$conscientiousness_score),var(male_subjects$conscientiousness_score), '\n')

cat('Srednja vrijednost Otvorenosti  za pripadnice muškog/ženskog spola je ', mean(female_subjects$openness_score), mean(male_subjects$openness_score),'\n')

cat('Varijanca vrijednost Otvorenosti  za pripadnice muškog/ženskog spola je ', var(female_subjects$openness_score),var(male_subjects$openness_score), '\n')

```
Nakon što usporedimo srednje vrijednosti prema svih 5 faktora, može se uočiti da između nekih postoji veća, odnosno manja razlika. Iako razlike nisu velike želimo ispitati da li je moguće procjeniti spol na temelju ovih faktora. Također najveća razlika se uočava između neurocizma, ugodnosti i otvorenosti. Dok je između savjesnosti i ekstraverzije gotovo zanemariva. Prikazali smo podatke grafički kako bi provjerili njihovu normalnost, te pokušali uočiti vizualne karakteristike.
...
Na ovom skupo podataka možemo uočiti razliku u srednjoj vrijednosti neurocizma između pripadnika ženskog i muškog spola. Stoga postoje indikacije da bi neuroticism_score trebao biti viši kod žena nego muškaraca, pa smo to provjerili pomoću T testa međutim prije samo testa moramo provjeriti nezavisnost i normalnost.

Sljedeći korak je provjeriti normalnost podataka koju najčešće provjeravamo: histgoramom, qq-plotom te KS-testom (kojim provjeravamo pripadnost podataka distribuciji). Sljedeće testove radimo samo za neuroticizam uz pretpostavku da se za ostale faktore podaci slično ponšaju.
...
```
S obzirom da je p-vrijednost < \(\alpha\), odbacujem nultu hipotezu u korist alternative da se neuroticizam muških/ženskih ispitanika NE ravna po normalnoj distibuciji, što je suprotno od onoga što smo uočili u grafovima. Uzevši u obzir oblik grafova i veliku osjetljivost Lillieforstovog testa normalnosti, možemo nastaviti testiranje uz pretpostavku normalnosti podataka.
...
Kako bi mogli provoditi T test uzorci moraju biti nezavisni i dolaziti iz normalne distribucije. Nezavisnost možemo utvrditi sa sigurnošću pošto je svaki ispitanik neovisan o drugom, odnosno svaki čovjek je zaseban pojedinac te njegovi rezultati ne ovise o razultatima drugog pojedinca. Normalnost podataka smo ispitali histogramima, qqplotovima i statističkim testovima.
...
Na temelju ovoga možemo zaključiti da je srenjda vrijednost neuroticizma za žene veća u odnosu na srednju vrijednost neuroticizma za muškarce. Ovo nam daje tračak nade da bi možda iz velikog skupa podataka možda mogli naučiti logistički model regresije da na temelju - Neuroticism_score,
                                         - Agreeableness_score,
                                         - Conscientiousness_score,
                                         - Extraversion_score,
                                         - Openness_score  uspijemo predvidjeti spol ispitanika. Stoga ćemo ispitati da li je to ostvarivo:

## Model logističe regresije za klasifikaciju spola ispitanika

Kako bi dobili binarnu varijablu za spol prilagodit ćemo varijablu tako da ćemo s 0 predstavljati muški spola, a s 1 ženski.
```{r}
data = bigfive
data$sex = data$sex -1
```
...
model = glm(sex ~ agreeable_score + extraversion_score + openness_score + conscientiousness_score + neuroticism_score, data = data, family = binomial())
summary(model)

Rsq = 1 - model$deviance/model$null.deviance
Rsq
```
Možemo uočiti da svaki score podjednako signifikantan, te provodimo analizu modela.
...
```
Model daje točnost od 0.6883763, što je bolje od slučajnog pogađanja spola. Međutim ne možemo tvrditi sa velikom vjerojatnosti da na temelju samo rezultata ispitivanja možemo predvidjeti spol. 
